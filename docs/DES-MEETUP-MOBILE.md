...
- Visibility row (first row): shows emoji + localized label (`🌍 Public`, `🔒 Private`, `👥 Group`) sourced via `LabelsContext` (`event_visibility.*`), shared with `CreateGameScreen` options via `getEventVisibilityLabel`. Group name is omitted on Event read view as group metadata is not present on the `Event` payload.
